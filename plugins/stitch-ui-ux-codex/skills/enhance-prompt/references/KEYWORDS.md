...
- Use a term only when it expresses user-provided intent, existing design-system
  evidence, or a justified product decision.
- Treat every term as a candidate vocabulary choice, never as a default mapping.
- Do not infer cards, sticky navigation, animation, gradients, glassmorphism,
  rounded containers, or other decorative UI from a vague noun or screen type.
- Prefer task, hierarchy, content, state, and responsive criteria over adjective
  bundles. Add at most the few terms that materially clarify the prompt.
